...
int8_t Screen::init( uint8_t backScrollPages, uint8_t initialWrapState_p ) {

  // Reasonable defaults for any video card.
  terminalLines = 25;
  terminalCols = 80;
  preventSnow = false;  // CGA only

  // Read the video mode from the BIOS area.  This always works.
  ...
  if ( mode == 7 ) {
    ...
    if ( (videoInt( 0x1a00, 0 ) & 0xFF) == 0x1a ) {
      cardType = 3; // VGA
    } else {

      // Not MDA or VGA.  If this does not return 0x10 it is an EGA BIOS.
      if ( getEgaMemSize( ) != 0x10 ) {
        cardType = 2; // EGA
      }

      // Ok, it is a CGA.  Which we already set above.
      cardType = 1;
    }

    // We are always using video modes 3 or 6 so this never changes with the card type.
    screenBaseSeg = 0xb800;

    // Only worry about this on CGA cards.
    if ( cardType == 1 ) {
  ...
  initialWrapState = initialWrapState_p;

  origCursor = readCursorShape( );
...
void Screen::restoreTextMode( void ) {

  if ( terminalLines == 25 ) {
    if ( cardType == 0 ) {
      setVideoMode( 7 );
    } else {
      // We don't care if this was an CGA, EGA or VGA card ...  this is simple.
      setVideoMode( 3 );
    }
  } else {

    // We support restoring 80x43 or 80x50.  Other modes, well, maybe another time.

    if ( cardType == 2 ) { // EGA

      videoInt( 0x0500, 0 ); // Set video page to 0.
      setVideoMode( 3 );

      if ( terminalLines == 43 ) {
        videoInt( 0x1112, 0 );    // Set the 8x8 font
        *((unsigned char far *)MK_FP( 0x40, 0x87 )) |= 1; // Cursor emulation enabled
      } else {
        // Just 25 lines
        videoInt( 0x1111, 0 );    // Set the 8x14 font
      }
...
void Screen::resetTerminalState( void ) {
  ...
  cursorSaveArea.autoWrap = initialWrapState;
  cursorSaveArea.lastColumn = false;
...
// Wrapping behavior:
//
// Ansi terminals implement "AM" which is "auto margin".  Auto margin means
// that when you write in the last column the next printable character will
// wrap around.  However, non-printable characters or escape codes should
// not cause that wrap around so you should not actually do the wrap around
// until you see the next printable character.

void Screen::addPrintableChar( char c ) {
  ...
  bool postAdvanceCursor = true;

  if ( cursor_x == (terminalCols-1) ) {
    if ( lastColumn == false ) {
      lastColumn = true;
      postAdvanceCursor = false;
    } else {
      if ( autoWrap ) {
        lastColumn = false;
        cursor_x = 0;
  ...
  // Technically we should call ScrOffset twice because it checks for wrap-around.
  // However we know we always allocate the buffer using an even number of bytes
  // so if there is a wrap-around it happens on the first call, and then we are
  // safe to increment it once as we do here.

  uint16_t o = ScrOffset(cursor_x, cursor_y );
  *((uint16_t far *)(buffer+o)) = ch;

  if ( postAdvanceCursor ) cursor_x++;
...
void Screen::add( char *buf, uint16_t len ) {
  ...
  for ( uint16_t i=0; i < len; i++ ) {
    ...
    // Fast path: check for printable characters first as these are
    // the most common.
    ...
    if ( (c > 31) && (c != 127) ) {
      ...
      switch ( c ) {
        ...
        case 127: { // Delete
          ...
          // Do not allow backspace back up to a previous row; no terminal
      ...
      // If it was in the control character range and we didn't recognize it
      // then don't worry about it.  Those are supposed to be non-printable.

      // Update the pointer into the video buffer assuming that processing the
      // control characters requires it.  Not all control characters do, but
      // this is cleaner that duplicating the code many times.
      updateVidBufPtr( );
    }
...
// putch only touches the real screen - it's meant for throwing
// characters and dialog boxes up in specific places.  You will
// need to repaint the screen after you are done.

void Screen::putch( uint8_t x, uint8_t y, uint8_t attr, char ch ) {
...
// The AutoWrap flag is not saved or restored, matching Xterm (and almost all real terminal) behavior.
...
void Screen::saveCursor( void ) {
  ...
  cursorSaveArea.lastColumn = lastColumn;
}
...
void Screen::restoreCursor( void ) {
  ...
  lastColumn = cursorSaveArea.lastColumn;
}
